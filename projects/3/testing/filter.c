...
IpPktFilter CreateFilter(void)
{
   ...
   fltCfg.blockedInboundTcpPorts = calloc(10, sizeof(unsigned int));
   fltCfg.numBlockedIpAddresses = 0; 
   fltCfg.blockedIpAddresses = calloc(10, sizeof(unsigned int));
   filter = &fltCfg;
   return filter; 
}
...
bool ConfigureFilter(IpPktFilter filter, char* filename)
{
   ...
   while(1)
   {
      ...
      if(strcmp(pToken, "BLOCK_IP_ADDR") == 0)
      {
         ParseRemainderOfStringForIp(ipAddr);
         temp = ConvertIpUIntOctetsToUInt(ipAddr);
      ...
      if(strcmp(pToken, "BLOCK_INBOUND_TCP_PORT") == 0)
      {
         pToken = strtok(NULL, ":");
         sscanf(pToken, "%u", &temp);
         dstTcpPort = temp;
...
bool FilterPacket(IpPktFilter filter, unsigned char* pkt)
{
   ...
   FilterConfig *fltCfg = (FilterConfig*)filter;
   unsigned int src = (pkt[12] << 8) + (pkt[13] << 8) + (pkt[14] << 8) + pkt[15];
   unsigned int dst = (pkt[16] << 8) + (pkt[17] << 8) + (pkt[18] << 8) + pkt[19];

   /// if we are supposed to block the source IP
   if(BlockIpAddress(fltCfg, src))
   {
      return false;
   }

   /// if we are supposed to block the destination ??
   if(BlockIpAddress(fltCfg, dst))
   {
      return false;
   }


   /// additional processing for ICMP and TCP packets
   if(pkt[9] == 1 || pkt[9] == 6)
   {
      ...
      if(pkt[9] == 1)
      {
         if(PacketIsInbound(fltCfg, src, dst))
         {
            return false;
         }
         return true;
      ...
      /// if protocol is TCP
      if(BlockInboundTcpPort(fltCfg,((pkt[26] << 8) + (pkt[27]))))
      {
         return false;
      }
      return true;
   ...
   /// when no conditions are met, we permit the packet
   return true;
}
...
{
   unsigned int localMask = fltCfg->localMask;
   unsigned int localIpAddr = fltCfg->localIpAddr;

   if(((dstIpAddr << localMask) & (localIpAddr <<  localMask)) == 0)
   {
      if(((srcIpAddr << localMask) & (localIpAddr << localMask)) != 0)
      {
         return true;
      }
   }
